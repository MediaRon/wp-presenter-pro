		...
		<div class="wrap">
			...
			<form method="POST">
				...
				<div id="wppp-gradients">
					...
					<?php
					wp_nonce_field( 'save_wppp_gradients', 'wppp_ajax_gradients_nonce' );
					?>
				<?php
				...
				foreach ( $gradients as $style => $name ) {
					...
					?>
					<button class="wppp-gradient <?php echo esc_attr( isset( $gradient_options[ $slug ] ) ? 'checked' : 'unchecked' ); ?>" arial-label="<?php echo esc_attr( $name ); ?>" title="<?php echo esc_attr( $name ); ?>" data-title="<?php echo esc_attr( $slug ); ?>" data-name="<?php echo esc_attr( $name ); ?>" data-style="<?php echo esc_attr( $style ); ?>"><span style="background-image: <?php echo esc_attr( $style ); ?>;"></span></button>
					<?php
				}
				?>
					<div><br />
